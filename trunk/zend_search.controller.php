...
require_once(_XE_PATH_ . 'modules/zend_search/lib/Zend/Zend_Search_Lucene_includer.php');
 ...
 **/
class zend_searchController extends zend_search {
    ...
    function createOrRetrieveIndex($index = self::INDEX_PATH_COMMENTS, $refresh=false) {
        ...
        $path = _XE_PATH_ . 'files/zend_search/';
    ...
    function reIndex($what='all', $chunks=200, $commentChunks=400, $timeLimit=600)
    {
        ...
        if ($what == 'documents' || $what == 'all') {
            ...
            $output = executeQuery('zend_search.getAllDocumentsSrls');
        ...
        if ($what == 'comments' || $what == 'all') {
            ...
            $output = executeQuery('zend_search.getAllCommentsSrls');
            $bunchOfSerials = array();
            foreach ($output->data as $i=>$obj) {
